...
def variance_configs(side_probabilities):
    ...
    for side_prob in side_probabilities:
        ...
        trainer_conf.update(
            logger=logger,
            work_dir='./training_logs_{}/run_/variance_{:.5f}/'.format(timestamp, side_prob)
        )
...
def training_sample_configs(num_samples):
    ...
    for n in num_samples:
        ...
        trainer_conf.update(
            logger=logger,
            work_dir='./training_logs_{}/run_/nsamples_{}/'.format(timestamp, n)
        )
        trainer_conf['train_data']['dataset'] = train_data
        trainer_conf['val_data']['dataset'] = val_data
        trainer_conf['additional_data']['s_randomized']['dataset'] = train_data.randomize_axes((0,))
...
def network_arch_configs(num_layers, latent_dims):
    ...
    for n_layers in num_layers:
        for latent_dim in latent_dims:
            ...
            trainer_conf.update(
                logger=logger,
                work_dir='./training_logs_{}/run_/nlayers_{}_latentdim_{}/'.format(timestamp, n_layers, latent_dim)
            )
...
def input_dim_configs(input_dims):
    ...
    for input_dim in input_dims:
        ...
        trainer_conf.update(
            logger=logger,
            work_dir='./training_logs_{}/run_/inputdim_{}/'.format(timestamp, input_dim)
        )
...
def num_slabs_configs():
    ...
    trainer_conf.update(
        logger=logger,
        work_dir='./training_logs_{}/run_/nslabs_{}/'.format(timestamp, 5)
    )
    trainer_conf['train_data']['dataset'] = train_data
    trainer_conf['val_data']['dataset'] = val_data
    trainer_conf['additional_data']['s_randomized']['dataset'] = train_data.randomize_axes((0,))
